...
async function getMusicByGenre(genre) {
...
router.get("/events", async (req, res) => {
    ...
    const emailSections = sections.map(section => {
        ...
        for (const event of section.events) {
            if (seen.has(event.booking_url))
                continue;
            if (alreadySent.has(event.booking_url))
                continue;
...
`;
    const totalEvents = emailSections.reduce((sum, section) => sum + section.events.length, 0);
    if (totalEvents === 0) {
    ...
    for (const section of emailSections) {
        for (const event of section.events) {
            await pool.query("INSERT INTO emailed_events (booking_url) VALUES ($1) ON CONFLICT DO NOTHING", [event.booking_url]);
        }
    }
